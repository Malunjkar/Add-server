    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Uploaded Ads</h1>
                        <?php 
                        include "../config.php";
                        $select = "SELECT new_ad.email, new_ad.adname, new_ad.imagepath, new_ad.startdate, new_ad.enddate, new_ad.target_url, new_ad.ad_height, new_ad.ad_width, new_ad.amount, click_count.click_count, click_count.client_email 
                                   FROM new_ad 
                                   LEFT JOIN click_count ON new_ad.adname = click_count.adname 
                                   WHERE new_ad.email = ? 
                                   GROUP BY new_ad.adname";
                        if ($stmt = mysqli_prepare($conn, $select)) {
                            ...
                            while ($res = mysqli_fetch_assoc($result)) {
                                $counter++;
                                $adname = htmlspecialchars($res['adname']);
                                $imagepath = htmlspecialchars($res['imagepath']);
                                $clickcount = htmlspecialchars($res['click_count']);
                                $startdate = htmlspecialchars($res['startdate']);
                                $enddate = htmlspecialchars($res['enddate']);
                                $height = htmlspecialchars($res['ad_height']);
                                $width = htmlspecialchars($res['ad_width']);
                                $amount = '₹' . htmlspecialchars($res['amount']);
                                $target = htmlspecialchars($res['target_url']);
                                echo "<tr>
                                        <td>$counter</td>
                                        <td>$adname</td>
                                        <td><img src='$imagepath' alt='$adname' /></td>
                                        <td>$clickcount</td>
                                        <td>$startdate</td>
                                        <td>$enddate</td>
                                        ...
                                        <td>
                                            <form action='deletead.php' method='POST'>
                                                <input type='checkbox' name='d[]' value='$adname'>
